...
- `collect_match()` fetches the four core 500 detail pages (`shuju`/`ouzhi`/`touzhu`/`yazhi`) through `fetch_html_batch()`. The `requests` backend uses bounded concurrent workers with isolated sessions; the Playwright backend stays serial. Parsing, unified supplements, feature snapshots, and SQLite writes must remain serial unless explicitly redesigned.
- The HTTP session sets `trust_env = False`, so proxy-related behavior from the shell environment is intentionally ignored.
- Field collection follows the strategy `primary/existing -> playwright-cli -> anysearch` for the supported analysis dimensions. Search fallback summaries must ignore search-page UI/navigation noise such as `跳至内容`, `辅助功能反馈`, `隐私政策`, `使用条款`, and `Rewards`; missing external values should be recorded with source/quality notes, not silently invented.
...
- Web batch collect/predict/settle/delete actions accept `selected_match_ids`; when present, all downstream work including `apply_target_batch_strategy_to_issue()` must stay scoped to those matches and must not rewrite unselected latest runs. Batch deletion must clean related `feedback_logs`, `issue_top_picks`, `prediction_runs`, `feature_snapshots`, and `analyses` rows together with `matches`.
...
- As of the 2026-06-26 local SQLite snapshot, learning profile #45 is active. It is a `handicap_bucket_table` target strategy for Asian handicap recommendations, trained/evaluated on real settled historical rows with target hit rate >= 70% and action share >= 60%. The full-history current-strategy replay has 1232 samples, 964 actions, 78.25% action share, and 74.27% hit rate. The homepage "historical feedback" figures are saved prediction feedback from mixed older profiles and must not be read as the active strategy replay.
